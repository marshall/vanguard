...
log_cmd="tail -f /tmp/vg-station.log | $this_dir/node_modules/bunyan/bin/bunyan -o short || sleep 100"
...
run tmux new-session -d -n "$window_name" \
  -s "$session_name" -c "$this_dir" "$time_cmd"
...
run tmux split-window -t $start_pane -v \
  -p $(( $shell_row_height + $log_row_height )) -c "$this_dir" "$shell_cmd"

run tmux split-window -t $shell -v \
  -p $(( $log_row_height * 2 )) -c "$this_dir" "$log_cmd"

top2_height=$(( $chart_row_height + $telemetry_row_height ))

run tmux split-window -t $start_pane -v \
  -p $(( $chart_row_height * 100 / $top2_height )) \
  -c "$this_dir" "$alt_chart_cmd"

run tmux split-window -t $start_pane -h -p $stats_width -c "$this_dir" "$stats_cmd"

leftover=$(( $time_width + $position_width ))
run tmux split-window -t $start_pane -h \
  -p $(( $position_width * 100 / $leftover )) -c "$this_dir" "$position_cmd"

run tmux split-window -t $alt_chart -h -p $temp_chart_width \
  -c "$this_dir" "$temp_chart_cmd"
